  ...
  <div class="row col-12 mb-2">
    <div class="create-user card">
      <div class="card-header d-flex justify-content-between">
        <div class="col-8">
          <strong class="page-title">メンバー登録</strong>
        </div>
...
export default {
  ...
  methods: {
    initial() {
      this.form.teamId = window.location.href.split('/').pop();
    },
    saveUser() {
      ...
      if (isValid) {
        axios
          .post('/api/admin/saveMember', this.form)
          .then(response => {
            this.$router.push({ path: '/sports/team_list/' + this.form.teamId });
            Vue.toasted.success('チームメンバーが正常に保存されました');
          })
          .catch(error => {
